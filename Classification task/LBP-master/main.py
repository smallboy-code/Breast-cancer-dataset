...
if __name__ == '__main__':
    ...
    angle = [0]
    print('-------------------1. Load Data---------------------')
    path = r"E:\Data\Mydataset\all\crop\alls\fold4\test"
    for type in os.listdir(path):
        type_path = os.path.join(path, type)
        for jpg in os.listdir(type_path):
            image = os.path.join(type_path, jpg)
            img = np.array(Image.open(image).convert('L'))  # If the image has multi-bands, it needs to be converted to grayscale image
            ...
            new_path = os.path.join(r'E:\Data\Mydataset/all\crop\GLCM2\test', type)
            if not os.path.exists(new_path):
                os.makedirs(new_path)
            plt.figure()
            plt.axis('off')
            plt.imshow(homogeneity, cmap='gray')
            plt.savefig(os.path.join(new_path,jpg), bbox_inches='tight', pad_inches=0,dpi=20)
